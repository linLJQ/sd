 $(function(){ //调用插件
    ...
    $(function(){
        form1 = $('form[name=form1]'),
            ...
            xiao = $('input[name=cho_Xiaoqu]',form1),
            vale = ['请选择省份','请选择城市','请选择地区','请选择街道','请选择小区'];
...
$.fn.citySelect = function(options) {  
 	var defaults = {  
 		setId : ['#Province' , '#City' , '#Area','#Insurer','#xiaoqu'],                //默认id
 		stval : ['请选择省份' , '请选择城市' , '请选择地区','请选择街道','请选择小区'],     //默认值
 		czemt : 'i',                        					  //存值元素
 	...
 	$.fn.appendlist = function(options){  //添加
 		...
 		for(var i = 0; i<appendArray.length;i++){
 			appList += '<li><a href="javascript:void(0)" title="'+appendArray[i]+'">'+appendArray[i]+'</a></li>';
 		}
 	...
 	$.fn.liClick = function(){
 			...
 			$('li' , _setId[3]).live('click', function(){ //街道
 				indC = $('li' , _setId[3]).index(this)-1;
 				var   _valD = $('a' , this).attr('title'),
 					_emeltD = $(_czemt , _setId[3]),
 					_inputD = $(_inpnt , _setId[3]);
 				_emeltD.text(_valD);
 				_inputD.val(_valD);
				$(_setId[4]).removelist({thisindex : 4});
				$(_setId[4]).appendlist({theindex:'0_'+indA+'_'+indB+'_'+indC+'_'+indD});
 				return indD;
 			});
			$('li' , _setId[4]).live('click', function(){ //街道
				indC = $('li' , _setId[4]).index(this);
				var   _valE = $('a' , this).attr('title'),
					_emeltE = $(_czemt , _setId[4]),
					_inputE = $(_inpnt , _setId[4]);
				_emeltE.text(_valE);
				_inputE.val(_valE);
